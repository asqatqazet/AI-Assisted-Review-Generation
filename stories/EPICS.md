...
product behavior. OpenAI/Gemini/Fake adapters and the deterministic Prompt
compose/request/grounding Evaluation, Candidacy and Deployment path are implemented, but that offline
evaluation explicitly does not measure provider behavior. The strict-$0 release executes **Generate only**
with FakeProvider. Paraphrase and the other transformation Actions, Experiments, paid-provider qualification,
public live-provider evidence and nonessential Console views remain deferred until their independent
validators, D0 and explicit provider credit resolve them.
...
| Story | User story | Acceptance evidence |
|---|---|---|
| US-03.1 Assertion capture and Format choice | As a reviewer, I can Generate from selected Fact Options/free text, then choose only compatible Review Formats. | Under the D7 default, rating 1-5 plus at least one non-rating Assertion enables Generate; release availability is the intersection of the production capability resolver, Tenant Action enablement, locale, Review Format support and required inputs; multi-Format cap is effective configuration. Paraphrase is a future story and is not reachable in the strict-$0 release. |
| US-03.2 Atomic paid-work admission | As a Tenant owner, I need duplicate clicks, retries, crashes and concurrent Lambdas to create one Generation Batch and at most one bounded Provider Attempt per planned Generation. | Under ADR-005's amendment to ADR-004, PostgreSQL atomically enforces Review Session/Tenant windows, active counts and Batch worst-case reservation; each child permit binds issuer/audience/expiry/JTI, Tenant, Location, Review Session, Batch/Generation, Action, Review Format Version, Assertions/request hash, snapshot id/hash and idempotency key; Generation uses DB time to verify permit expiry while creating one finite no-provider lease, and Context activates only its valid signed receipt; before the one strict-$0 provider call, Generation uses DB time to verify activation and atomically changes `LEASED -> RUNNING` while claiming Attempt ordinal 1, so parallel/replayed losers only tail the existing execution; after deterministic validation, a fenced `CHECKPOINTED` transaction stores raw provider output, separate receipt/usage and sanitized classification before an idempotent terminal transaction creates the successful or rejected Generation; crash-after-checkpoint recovery never calls the provider again, conflicting replay fails, and an uncheckpointed timeout remains indeterminate and unreleased; Context settles only a valid signed terminal receipt within the reservation. |
...
| US-03.4 Complete grounding and policy | As a reviewer, I need every factual/evaluative proposition in the Draft represented by a Claim supported by my Assertions or narrowly permitted verified context; typed system annotations stay separate. | Under accepted D9, structural/adversarial tests cover incomplete Claim maps, wrong Tenant/Review Session/version/span, rating overreach, context allow-list, banned terms, disclosure provenance and post-rewrite revalidation; the defined semantic-support validator and golden set measure—not universally prove—entailment; any unsupported candidate rejects the whole result, creates no Draft, and its protected raw provider evidence never crosses the reviewer/ordinary Console service boundary. |
| US-03.5 Deferred transformation capabilities | As a future reviewer, I can Paraphrase, Resample, Reformat, Condense, Expand, Revise Wording or Add Assertion without silently adding facts. | This story is not in the strict-$0 release. Each Action needs an independently release-qualified semantic validator: Paraphrase proves no new and no silently dropped source propositions; Resample reuses original normalized inputs/versions; Reformat/Condense are source-Claim subsets; Expand preserves the exact Claim set or rejects; Revise Wording adds none; Add Assertion requires explicit confirmation and reruns all checks. Manual Draft edits cannot become grounding through a retry path. |
| US-03.6 Draft, copy and disposition | As a reviewer, I can edit, compare and copy my Draft while immutable Generation evidence stays intact. | Edits create Draft revisions, never mutate Generation; the editable body and immutable typed disclosure annotations are separate, every revision inherits the annotations, and copy composes them without turning system provenance into reviewer text; over-limit reviewer text warns rather than blocks; copy awaits success and has an accessible select/manual fallback; outbound URL comes from the exact Location x Review Format binding and opens separately; `done` may start another admitted compatible Format without reusing the Invitation Token; Disposition records no publication claim. |
...
Prompt Evaluation Results, append-only Candidacy Decisions and Prompt Deployment are part of the implemented
configuration publication boundary. Experiments, broad analytics and provider-management screens are
separate deferred differentiators, not prerequisites for basic Tenant operation. Create explicit conditional
stories if D0 proves the assignment requires them; do not hide them inside US-04 acceptance.
...
| Story | User story | Acceptance evidence |
|---|---|---|
| US-06.1 Day-one AWS walking skeleton | As the assignment reviewer, I can open an access-restricted non-production health/UI path without risking an AWS bill. | After D2/D3/D11, deployment preflight proves a new-account Free plan, records teardown date, and records one explicit capacity profile end to end. `student-low-quota` requires at least 10 account/unreserved concurrency, omits function reservations and is restricted to synthetic Tenant data/FakeProvider; `reserved-concurrency` allocates BFF fast/stream 5/2, Reviewer/Console Context 4/1 and Generation 1 (13 total) while preserving AWS's required 100 unreserved. Terraform creates CloudFront/default HTTPS, private S3, OAC-protected fast/stream BFF Function URLs, EventBridge-only reconciliation, two private Context Lambdas from one logical deployable, private Generation, Neon Free, Cognito prefix domain, SSM Standard secrets and IAM; direct Function URLs return 403; a real 60-second cloud smoke remains release evidence, not local evidence. |
| US-06.2 Secure immutable delivery | As an operator, I need each release traceable and no long-lived cloud credentials. | GitHub OIDC is repo/branch/environment scoped; `pnpm verify` precedes deploy; a release manifest identifies UI and three Lambda zip artifacts with separate checksums and coordinated order; six qualified `candidate`/`live` functions and hashed UI assets identify the release; `$LATEST`, dummy zips and plaintext placeholder keys are absent. The first Terraform bootstrap is explicitly not candidate-before-live evidence. |
...
|---|---|---|
| `entry` | Clean `/start/:entryChallengeHandle` rating and Generate entry; explicit Start creates the Review Session and redirects to `/review/:reviewSessionHandle`. Paraphrase choice is a future-state fixture, not a strict-$0 route. | US-01.2, US-01.3, US-02.1, US-02.2 |
...
| `paraphrase-input` | Future compatibility state: reviewer source text would be the sole factual source and revision/span anchored; currently unreachable in the release capability resolver | Deferred US-03.5 |
...
| `results-grounding-stripped` | Compatibility label for accepted `results-partial`: safe sibling Drafts survive beside a generic rejected card; no Unsupported Output bytes; Add Assertion remains part of deferred US-03.5 | US-03.3-US-03.5 |
...
| `prompts` | Tenant-owned immutable Prompt Versions plus append-only Evaluation/Candidacy evidence and current Deployment pointer | Implemented under US-04.2/US-05.4 |
| `experiments` | Stable future Experiment assignment over qualified Prompt Versions | Deferred; requires an explicit story and release evidence if D0 requires it |
...
| R3 — reviewer control + operators | Generate Draft lifecycle and scoped Console, including Prompt candidacy/deployment | US-03.6, US-04.1-US-04.5 |
| R3F — transformation capabilities | Conditional, validator-gated Paraphrase/Resample/Reformat/Condense/Expand/Revise Wording/Add Assertion | Deferred US-03.5 |
